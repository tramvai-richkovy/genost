...
import { MidiRollPreview } from "../midi-generator/MidiRollPreview";
import {
...
function MidiPreview({ path, name }: { path: string | null; name: string }) {
  return <MidiRollPreview name={name} path={path} toAssetUrl={(value) => sourceForPreview(value) ?? value} />;
}
  ...
  return (
    <div className="setup-strip">
          ...
          <select
            ...
            onChange={(event) => {
              void updateModelSettings({
                ...settings,
                backend: event.currentTarget.value as typeof settings.backend,
              }).then(() => checkPreflight());
            }}
      ...
      <div className="preflight-grid">
        ...
        {Object.entries(workerPreflight?.capabilities ?? {}).map(([name, capability]) => (
          <span
            className={`status-pill ${capability.available ? "ready" : "warning"}`}
            key={name}
            title={capability.available ? capability.detail : capability.setup_hint ?? capability.detail}
          >
            {capability.available ? <Cpu size={14} /> : <Unplug size={14} />}
            {name.replace(/_/g, " ")}
          </span>
        ))}
      </div>
  ...
  if (collapsed) {
    return (
      <aside className="session-sidebar collapsed">
        <button aria-label="Expand sidebar" className="icon-button" onClick={() => void setSidebarCollapsed(false)} title="Expand sidebar" type="button">
          <Menu size={18} />
        </button>
        <button aria-label="New session" className="icon-button" disabled={!canUseStudio} onClick={() => onCreate("free_format")} title="New session" type="button">
          <Plus size={18} />
        </button>
      </aside>
    );
  ...
  return (
    <aside className="session-sidebar">
      <div className="sidebar-head">
        <button aria-label="Collapse sidebar" className="icon-button" onClick={() => void setSidebarCollapsed(true)} title="Collapse sidebar" type="button">
          <Menu size={18} />
        </button>
        ...
        <button aria-label="Refresh sessions" className="icon-button" onClick={() => void refreshSessions()} title="Refresh sessions" type="button">
          <RefreshCcw size={17} />
        </button>
      </div>
  ...
  return (
    <form className="work-panel session-form" onSubmit={(event) => void submit(event)}>
      <div className="panel-title-row">
        ...
        <button aria-label="Close new session form" className="icon-button" onClick={onDone} title="Close" type="button">
          <X size={17} />
        </button>
      </div>
    ...
    void Promise.allSettled(
      sessions.map(async (card) => {
        ...
        return loaded.session.artifacts
          ...
          .map((artifact) => {
            const revision = loaded.session.promptHistory.revisions.find(
              (item) => item.id === artifact.promptRevisionId,
            );
            return {
              ...
              revisionLabel: revision?.label ?? "unassigned",
              revisionFolder: revision?.artifactFolder ?? artifact.filePath.split("/")[0] ?? "artifacts",
            };
          });
      }),
    )
...
function PromptComposer({
  ...
  const [selectedRevisionId, setSelectedRevisionId] = useState(activeRevision.id);
  const [quantity, setQuantity] = useState(mode === "midi" ? 4 : 2);
  const selectedRevision =
    session.promptHistory.revisions.find((revision) => revision.id === selectedRevisionId) ?? activeRevision;
  const viewingActiveRevision = selectedRevision.id === activeRevision.id;

  useEffect(() => setSelectedRevisionId(activeRevision.id), [activeRevision.id]);
    ...
    <div className="composer-panel">
      <div className="composer-toolbar">
        <div className="revision-tabs">
          {session.promptHistory.revisions.map((revision) => (
            <button
              className={`revision-tab ${revision.id === selectedRevision.id ? "active" : ""}`}
              key={revision.id}
        ...
        <button aria-label="Archive prompt revision" className="icon-button" onClick={() => void archivePrompt()} title="Archive prompt revision" type="button">
          <Archive size={17} />
        </button>
      </div>
      <textarea
        className="prompt-input"
        disabled={!viewingActiveRevision || activeRevision.locked}
        ...
        value={viewingActiveRevision ? prompt : selectedRevision.prompt}
      />
      {children}
      <div className="composer-actions">
        ...
        <button
          ...
          disabled={generateDisabled || !viewingActiveRevision || activeRevision.locked}
...
function MidiGeneratorView({ session }: { session: GenostSession }) {
  ...
  const text2midi = useSessionStudioStore((state) => state.workerPreflight?.capabilities.text2midi);
  const activeRevision = activePromptRevision(session);
  ...
  return (
    <PromptComposer
      mode="midi"
      generateDisabled={!text2midi?.available}
      generateTitle={text2midi?.available ? "Generate MIDI" : text2midi?.setup_hint ?? text2midi?.detail}
...
function ReferencePicker({ entries }: { entries: ArtifactTreeEntry[] }) {
  ...
  const hierarchy = useMemo(() => {
    const sessions = new Map<string, { title: string; revisions: Map<string, ArtifactTreeEntry[]> }>();
    for (const entry of entries) {
      const session = sessions.get(entry.sessionId) ?? {
        title: entry.sessionTitle,
        revisions: new Map<string, ArtifactTreeEntry[]>(),
      };
      const revisionKey = entry.revisionId ?? `${entry.revisionFolder}:unassigned`;
      session.revisions.set(revisionKey, [...(session.revisions.get(revisionKey) ?? []), entry]);
    ...
    <div className="reference-panel">
      <div className="reference-actions">
        ...
        {selectedReferenceAudio ? (
          <button aria-label="Clear reference" className="icon-button danger" onClick={clearReferenceAudio} title="Clear reference" type="button">
            <X size={17} />
          </button>
        ) : null}
      </div>
      ...
      {openTree ? (
        <div className="artifact-tree">
          {hierarchy.map(([sessionId, session]) => (
            <details key={sessionId} open>
              <summary>{session.title}</summary>
              {[...session.revisions.entries()].map(([revisionId, revisionEntries]) => (
                <details key={revisionId}>
                  <summary>
                    {revisionEntries[0].revisionLabel} · {revisionEntries[0].revisionFolder}
                  </summary>
                    ...
                    <button
                      ...
                      onClick={() =>
                        useResolvedArtifactAsReference({
                          ...
                          name: `${entry.sessionTitle}/${entry.revisionLabel}/${entry.artifactName}`,
                          source: "artifact",
...
function StemConstructorView({ session, artifactTree }: { session: GenostSession; artifactTree: ArtifactTreeEntry[] }) {
  ...
  const selectedRevision =
    session.promptHistory.revisions.find((revision) => revision.id === selectedRevisionId) ?? activeRevision;
  const viewingActiveRevision = selectedRevision.id === activeRevision.id;
  const editingDisabled = !viewingActiveRevision || activeRevision.locked;
    ...
    <div className="composer-panel">
      ...
      <div className="constructor-grid">
        <label className="field-group">
          ...
          <select className="field" disabled={editingDisabled} onChange={(event) => setConstructorId(event.currentTarget.value)} value={constructorId}>
        ...
        {constructor.fields.map((field) => (
          <label className="field-group" key={field.id}>
            ...
            <input className="field" disabled={editingDisabled} list={`${constructor.id}-${field.id}`} onChange={(event) => setValues((current) => ({ ...current, [field.id]: event.currentTarget.value }))} value={values[field.id] ?? ""} />
      ...
      <div className="prompt-readout">{viewingActiveRevision ? prompt : selectedRevision.prompt}</div>
      <ReferencePicker entries={artifactTree} />
      <div className="composer-actions">
        <label className="field-group compact">
          <span>Quantity</span>
...
function ArtifactCard({ artifact, selected, toggleSelected }: { artifact: GenostArtifact; selected: boolean; toggleSelected: (artifactId: string) => void }) {
  ...
  const cancelArtifactJob = useSessionStudioStore((state) => state.cancelArtifactJob);
  const retryAudioArtifact = useSessionStudioStore((state) => state.retryAudioArtifact);
  const capabilities = useSessionStudioStore((state) => state.workerPreflight?.capabilities ?? {});
    ...
    <article className={`artifact-card ${artifact.status}`}>
        ...
        {artifact.provenance ? (
          <>
            <span>
              ...
              {artifact.provenance.model ? ` · ${artifact.provenance.model}` : ""}
              {artifact.provenance.seed !== null ? ` · seed ${artifact.provenance.seed}` : ""}
            </span>
            {artifact.provenance.exactPrompt ? <span title={artifact.provenance.exactPrompt}>{artifact.provenance.exactPrompt}</span> : null}
          </>
        ) : null}
      </div>
      {isAudio ? (
      ...
      <div className="artifact-actions">
        {artifact.status === "queued" || artifact.status === "generating" ? (
          <button aria-label="Cancel generation" className="icon-button danger" onClick={() => void cancelArtifactJob(artifact.id)} title="Cancel generation" type="button">
            <X size={15} />
          </button>
        ) : null}
        {(artifact.status === "failed" || artifact.status === "canceled") && artifact.provenance?.operation === "musicgen" ? (
          <button aria-label="Retry as a new artifact" className="icon-button" onClick={() => void retryAudioArtifact(artifact.id)} title="Retry as a new artifact" type="button">
            <RefreshCcw size={15} />
          </button>
        ) : null}
        <button aria-label="Reveal artifact location" className="icon-button" disabled={!absolutePath || artifact.status !== "ready"} onClick={() => void reveal()} title="Reveal location" type="button">
          <ExternalLink size={15} />
        </button>
        <button aria-label="Export artifact" className="icon-button" disabled={artifact.status !== "ready"} onClick={() => void exportArtifact(artifact.id)} title="Export artifact" type="button">
          <Download size={15} />
        </button>
        {isAudio ? (
          <>
            <button aria-label="Use artifact as reference" className="icon-button" disabled={artifact.status !== "ready"} onClick={() => useArtifactAsReference(artifact.id)} title="Use as reference" type="button">
              <Play size={15} />
            </button>
            <button aria-label="Split artifact into stems" className="icon-button" disabled={artifact.status !== "ready" || !capabilities.separator?.available} onClick={() => void splitArtifactIntoStems(artifact.id)} title={capabilities.separator?.available ? "Split to stems" : capabilities.separator?.setup_hint ?? "Separator unavailable"} type="button">
              <SlidersHorizontal size={15} />
            </button>
            <button aria-label="Convert artifact to melodic MIDI" className="icon-button" disabled={artifact.status !== "ready" || !capabilities.basic_pitch?.available} onClick={() => void convertArtifactToMidi(artifact.id, "melodic")} title={capabilities.basic_pitch?.available ? "Convert to melodic MIDI" : capabilities.basic_pitch?.setup_hint ?? "Basic Pitch unavailable"} type="button">
              <Music2 size={15} />
            </button>
            <button aria-label="Convert artifact to drum MIDI" className="icon-button" disabled={artifact.status !== "ready" || !capabilities.omnizart?.available} onClick={() => void convertArtifactToMidi(artifact.id, "drum")} title={capabilities.omnizart?.available ? "Convert to drum MIDI" : capabilities.omnizart?.setup_hint ?? capabilities.omnizart?.detail ?? "Omnizart unavailable"} type="button">
              <AudioLines size={15} />
            </button>
            <button className="control-button" disabled={artifact.status !== "ready" || !capabilities.separator?.available || !capabilities.merge?.available || !capabilities.basic_pitch?.available || !capabilities.midi_guide?.available} onClick={() => void createDerivedSessionFromArtifact(artifact.id, "free_format")} type="button">
        ...
        {isMidi ? (
          <>
            <button className="control-button" disabled={artifact.status !== "ready" || !capabilities.midi_guide?.available} onClick={() => void createDerivedSessionFromArtifact(artifact.id, "free_format")} type="button">
              Free Format
            </button>
            <button className="control-button" disabled={artifact.status !== "ready" || !capabilities.midi_guide?.available} onClick={() => void createDerivedSessionFromArtifact(artifact.id, "stem_constructor")} type="button">
              Stem Constructor
            </button>
          </>
        ) : null}
...
function JobQueue() {
  const jobs = useSessionStudioStore((state) => Object.values(state.workerJobs));
  const cancelArtifactJob = useSessionStudioStore((state) => state.cancelArtifactJob);
  const visible = jobs.filter((job) => !["ready", "failed", "canceled"].includes(job.status));
  ...
  return (
    <section className="composer-panel">
      <div className="artifact-section-head">
        <h2>Render Queue</h2>
        <span className="status-pill">{visible.length}</span>
      </div>
      {visible.map((job) => (
        <div className="queue-row" key={job.job_id}>
          ...
          <button
            ...
            onClick={() => void cancelArtifactJob(job.job_id.replace(/^job_/, ""))}
...
function ArtifactList({ session }: { session: GenostSession }) {
  ...
  const mergeCapability = useSessionStudioStore((state) => state.workerPreflight?.capabilities.merge);
  ...
  return (
    <div className="artifact-section">
      <div className="artifact-section-head">
        <h2>Artifacts</h2>
        <button className="control-button" disabled={selectedStems.length === 0 || !mergeCapability?.available} onClick={() => void mergeSelectedStems(selectedStems)} title={mergeCapability?.available ? "Merge selected stems" : mergeCapability?.setup_hint ?? "ffmpeg unavailable"} type="button">
          <AudioLines size={17} />
          Merge Selected
        </button>
      </div>
    ...
    <section className="cp-root genost-screen session-studio min-h-screen bg-genost-base text-genost-text">
      <header className="studio-topbar">
        ...
        <div className="topbar-actions">
          ...
          <button aria-label={theme === "dark" ? "Use light theme" : "Use dark theme"} className="icon-button" onClick={toggleTheme} title={theme === "dark" ? "Use light theme" : "Use dark theme"} type="button">
            <ThemeIcon size={18} />
          </button>
        </div>
      </header>
