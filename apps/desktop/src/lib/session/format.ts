...
export function createPromptRevision(args: {
  ...
  return {
    ...
    prompt: args.prompt ?? "",
    artifactFolder: args.artifactFolder ?? `${ARCHIVE_DIRECTORY_PREFIX}-1`,
...
export function createArtifact(args: {
  ...
  provenance?: GenostArtifact["provenance"];
  volumeDb?: number;
}): GenostArtifact {
  const createdAt = nowIso();
  return {
...
export function archivePromptRevision(session: GenostSession): GenostSession {
  ...
  const stableFolderLabel = new RegExp(`^${ARCHIVE_DIRECTORY_PREFIX}-\\d+$`).test(active.artifactFolder)
    ? active.artifactFolder
    : null;
  const archiveLabel = stableFolderLabel ?? nextArchiveLabel(session);
  ...
  const nextRevision = createPromptRevision({
    prompt: active.prompt,
    artifactFolder: nextRevisionFolder(session),
  });
...
export function nextRevisionFolder(session: GenostSession): string {
  const used = new Set(
    session.promptHistory.revisions.flatMap((revision) => [revision.artifactFolder, revision.label]),
  );
  for (let index = 1; index < 10_000; index += 1) {
    const candidate = `${ARCHIVE_DIRECTORY_PREFIX}-${index}`;
...
export function buildStemConstructorPrompt(args: {
  ...
  const values = constructor.fields
    ...
    .filter((value): value is string => Boolean(value))
    .filter((value) => !(constructor.id === "pad" && value.toLocaleLowerCase() === "atmospheric"));
