...
export async function createSessionOnDisk(rootPath: string, draft: CreateSessionDraft): Promise<LoadedSession> {
  ...
  const session = createEmptySession(draft);
...
async function writeLoadedSessionFiles(sessionPath: string, loadedSession: LoadedSession): Promise<void> {
  await ensureSessionFolders(sessionPath, loadedSession.session);
  await writeJsonAtomic(await join(sessionPath, SESSION_FILE_NAME), loadedSession.session);
  await writeJsonAtomic(await join(sessionPath, COMMANDS_FILE_NAME), loadedSession.commands);
}
