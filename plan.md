...
- Acceptance CLI structural audio validation that preserves dark-material results while recording complete spectral metrics for review; content-specific session-render validation remains unchanged.
- Packaged-worker Basic Pitch model data collection, discovered during the unsigned DMG launch walkthrough so the verified ONNX capability remains available outside the development virtual environment.
...
- `just verify` passes after the interruption-reconciliation changes: 28 desktop files passed and one skipped, 112 tests passed and seven intentionally skipped; the production TypeScript/Vite build and product-shell assertion passed; all 56 worker tests passed; and Rust formatting plus source compilation passed using the no-sidecar source-check configuration.
- The real isolated Basic Pitch smoke passed earlier on the checked Apple Silicon environment.
- Signal-safe manifest finalization and stale-process reconciliation are implemented, and all three earlier abandoned acceptance manifests have been reconciled to `status: "interrupted"` without removing their retained partial files.
- A real `just test` pass completed on MLX/Metal in `test-output/20260829T064453Z`: the ready manifest owns exactly five distinct seeded 25-second, 32 kHz WAVs and five sidecars. All files were re-read, decoded through ffmpeg, hash/seed checked, and inspected for duration, channels, finite samples, peak, RMS, DC, and spectral metrics. Subjective listening remains a release review because this environment cannot consume audio.
- The complete model-free worker suite now passes with 56 tests, including a real `SIGTERM` delivery test and dead/active/legacy acceptance-manifest reconciliation coverage.
- The documented unsigned packaging command now produces both `GENOST.app` and a checksum-valid Apple Silicon DMG. A packaged-app launch reached worker health with MLX/Metal, both required models, MIDI guide, merge, and the isolated Basic Pitch ONNX inference probe available.

Still required before the Definition of Done: complete subjective listening review of the five acceptance outputs, close the broader store/UI/storage coverage listed below, and perform a real Tauri working-directory walkthrough beyond the verified packaged startup/preflight smoke.
...
- [x] Run the real `just test` acceptance pass on the target Mac and inspect the integrity, identity, metadata, decoding, and measured audio properties of all five outputs.
- [ ] Complete subjective listening review of all five accepted WAVs before release.
- [x] Reconcile or finalize an acceptance batch when its process is externally terminated, retaining partial files while recording the signal or stale-process reason.
...
- [x] Run `just verify`, Rust format/check, package the unsigned macOS app, verify its DMG checksum, and smoke-launch its bundled worker/preflight.
- [ ] Perform a real packaged-app working-directory walkthrough through session creation/opening and artifact actions.
